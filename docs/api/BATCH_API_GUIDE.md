...
const response = await fetch('https://adblock.jaysonknight.com/compile/batch', {
    method: 'POST',
    headers: {
...
async function compileBatchAsync() {
    ...
    const queueResponse = await fetch(
        'https://adblock.jaysonknight.com/compile/batch/async',
        {
            method: 'POST',
    ...
    while (!results && attempts < maxAttempts) {
        ...
        const statusResponse = await fetch(
            `https://adblock.jaysonknight.com/queue/results/${requestId}`
        );
...
BASE_URL = 'https://adblock.jaysonknight.com'
...
curl -X POST https://adblock.jaysonknight.com/compile/batch \
...
# Step 1: Queue the batch
curl -X POST https://adblock.jaysonknight.com/compile/batch/async \
  -H "Content-Type: application/json" \
  -d '{
    "requests": [
      {
...
curl https://adblock.jaysonknight.com/queue/results/req-1704931200000-abc123
