...
const response = await fetch('https://adblock-compiler-ui.pages.dev/compile/batch', {
    method: 'POST',
    headers: {
...
async function compileBatchAsync() {
    ...
    const queueResponse = await fetch(
        'https://adblock-compiler-ui.pages.dev/compile/batch/async',
        {
            method: 'POST',
    ...
    while (!results && attempts < maxAttempts) {
        ...
        const statusResponse = await fetch(
            `https://adblock-compiler-ui.pages.dev/queue/results/${requestId}`
        );
...
BASE_URL = 'https://adblock-compiler-ui.pages.dev'
...
curl -X POST https://adblock-compiler-ui.pages.dev/compile/batch \
...
# Step 1: Queue the batch
curl -X POST https://adblock-compiler-ui.pages.dev/compile/batch/async \
  -H "Content-Type: application/json" \
  -d '{
    "requests": [
      {
...
curl https://adblock-compiler-ui.pages.dev/queue/results/req-1704931200000-abc123
