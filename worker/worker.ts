...
async function serveStaticAsset(request: Request, env: Env, pathname: string): Promise<Response> {
    if (env.ASSETS) {
        try {
            // For the root path, serve the Angular SPA shell. The postbuild script copies
            // index.csr.html (generated by RenderMode.Client builds) to index.html, so
            // index.html is the canonical name. Fall back to index.csr.html in case a build
            // was run without the postbuild step (e.g. ng build invoked directly).
            if (pathname === '/') {
                const htmlResponse = await env.ASSETS.fetch(new URL('/index.html', ASSETS_BASE_URL));
                if (htmlResponse.ok) {
                    return htmlResponse;
                }
                const csrResponse = await env.ASSETS.fetch(new URL('/index.csr.html', ASSETS_BASE_URL));
                if (csrResponse.ok) {
                    return csrResponse;
                }
            }

            const assetUrl = new URL(pathname, ASSETS_BASE_URL);
            ...
            if (!FILE_EXTENSION_RE.test(pathname) && !isServerPath && acceptsHtml) {
                // index.html is the canonical SPA shell (copied from index.csr.html by postbuild).
                // Fall back to index.csr.html if index.html is somehow absent.
                const spaHtml = await env.ASSETS.fetch(new URL('/index.html', ASSETS_BASE_URL));
                if (spaHtml.ok) {
                    return spaHtml;
                }
                const spaCsr = await env.ASSETS.fetch(new URL('/index.csr.html', ASSETS_BASE_URL));
                if (spaCsr.ok) {
                    return spaCsr;
                }
            }
