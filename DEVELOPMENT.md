...
2. Register the transformation in the `TRANSFORMATIONS` enum in `src/transformations/enum.js` — the single source of
   truth for transformation names; the configuration schema derives its accepted names from it. The declaration order
   matters (it renders in the unknown-transformation error message's allowed-values list), so insert the entry in the
   right place.
3. Register the transformation in `TRANSFORMATIONS_IN_ORDER` in `src/transformations/transform.js` — that list pairs
   each enum name with its implementation and determines execution order, not the order in the configuration. A test
   asserts the list covers every enum entry and pins its order, so a missing registration or an accidental reorder
...
5. If the transformation should be part of the quick `-i` conversion pipeline, add it to
   `DEFAULT_TRANSFORMATIONS` in `src/transformations/transform.js`.
6. Apply the documentation-sync rules from [AGENTS.md](AGENTS.md#configuration--documentation)
   (`src/schemas/configuration.schema.js`, `src/index.d.ts`, the order list in `README.md`, `CHANGELOG.md`).
7. Create a test file in `test/transformations/`.
8. Run `pnpm lint && pnpm test`.
